...
<div class="container py-4">
  ...
  <div class="d-flex gap-2 mb-3">
    ...
    <a href="create_live_session.php" class="btn btn-warning"><i class="fas fa-satellite-dish"></i> สร้างห้อง Live</a>
  </div>
          ...
          if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
              ...
              <tr>
                ...
                <td>
                  <div class="progress" role="progressbar" aria-label="Student progress" aria-valuenow="<?= $percentage ?>"
                    aria-valuemin="0" aria-valuemax="100" style="height: 25px; font-size: 1rem;">
                    <div class="progress-bar progress-bar-striped progress-bar-animated bg-success"
                      style="width: <?= $percentage ?>%"><?= round($percentage) ?>%</div>
                  </div>
                  <div class="text-center small text-muted mt-1"><?= $completed_stages ?>/<?= $total_stages ?> ด่าน</div>
                </td>
                <td class="text-center">
                  <a href='edit_user.php?id=<?= $row['id'] ?>' class='btn btn-warning btn-sm' title="แก้ไข"><i
                      class="fas fa-edit"></i></a>
                  <a href='delete_user.php?id=<?= $row['id'] ?>' class='btn btn-danger btn-sm' title="ลบ"
                    onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบผู้ใช้คนนี้?');"><i class="fas fa-trash"></i></a>
                </td>
              </tr>
              <?php
            }
